...
applyMove bs (Move (from, to)) = BoardState board wPieces bPieces
  where
    board = V.unsafeUpd
        (getBoard bs)
        [(indexToInt from, emptyField), (indexToInt to, fromField)]
...
getMoves ms =
    let (movetype, offsets) = getMoveInfo ms
    in  case movetype of
...
getSingleSlideMoves ms@(i, _, _) o =
    V.map (\dest -> Move (i, dest)) $ snd . V.unzip . V.unsafeTail $ V.takeWhile
        isPossibleSlide'
        destinations
  where
    destinations     = getPossibleSlidesFrom i o
...
isPossibleSlide ms@(_, c, bs) destinations (n, dest)
    ...
    = True
    | otherwise
    = not $ isIllegalDestination ms dest || isOpponentPiece c lastField
...
getPossibleSlidesFrom i o = V.constructN 10 iterator
  where
    iterator v = if V.null v
        then (0, i)
        else let (n, j) = V.last v in (n + 1, changeIndexBy j o)

-- | Returns information about a move
getMoveInfo :: MoveState -> (MoveType, V.Vector Offset)
getMoveInfo ms@(i, c, bs) =
    let fieldtype = getFieldType $ getField bs i
...
getPawnOffsets (i, c, bs) = V.force $ V.filter (/= notPossible) $ V.generate
    4
    gen
  where
    notPossible = 0
    ...
    gen 1 = if isPawnRowIndex c i && isEmpty front && isEmpty front2
        then colorize 20
        else notPossible
